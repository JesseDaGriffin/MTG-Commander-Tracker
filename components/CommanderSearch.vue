<template>
    <div class="relative w-full commander-search">
        <div class="relative flex items-center">
            ...
            <div
                v-if="isLoading"
                class="absolute right-4 top-1/2 -translate-y-1/2 text-accent-primary pointer-events-none"
            >
        ...
        <div
            ...
            class="absolute top-[calc(100%+0.5rem)] left-0 right-0 z-50 max-h-[400px] overflow-y-auto p-2 flex flex-col gap-1 card shadow-xl"
        >
            <div
            ...
            >
                <div
                    class="w-[50px] h-[70px] bg-bg-primary rounded-sm flex items-center justify-center overflow-hidden border border-border-color flex-shrink-0"
                >
                    <img
                        v-if="card.imageUrl"
                        :src="card.imageUrl"
        ...
        <!-- No Results State -->
        <div
            v-else-if="showResults && searchQuery.length >= 3 && !isLoading"
            class="absolute top-[calc(100%+0.5rem)] left-0 right-0 z-50 p-4 text-center card shadow-xl"
        >
            <p class="text-muted">
                No commanders found for "{{ searchQuery }}"
            </p>
        </div>
    </div>
</template>
...
const fetchCommanders = async (query) => {
    try {
        ...
        const scryfallQuery = encodeURIComponent(
            `t:legend t:creature ${query}`,
        );
        const response = await fetch(
            `https://api.scryfall.com/cards/search?q=${scryfallQuery}`,
        );
